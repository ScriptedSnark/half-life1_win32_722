...
//
	{
		unsigned short* pPal = (unsigned short*)Hunk_AllocName(sizeof(unsigned short) * 4 * 256, loadname);
		unsigned char* pPalSrc = (unsigned char*)pframetype + 2;

		for (i = 0; i < 256; i++)
		{
			pPal[i * 4 + 0] = pPalSrc[i * 3 + 2];
			pPal[i * 4 + 1] = pPalSrc[i * 3 + 1];
			pPal[i * 4 + 2] = pPalSrc[i * 3 + 0];
			pPal[i * 4 + 3] = 0;
		}
...
void Mod_LoadSpriteModel( model_t* mod, void* buffer )
{
	...
	size = sizeof(msprite_t) + (numframes - 1) * sizeof(psprite->frames) + (palsize * 4 * sizeof(unsigned short)) + 2;
...
//
	...
	pframetype = (dspriteframetype_t*)((byte*)(pin + 1) + 2 + palsize * 3);
