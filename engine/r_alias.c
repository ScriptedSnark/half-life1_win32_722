...
void R_AliasTransformVector( vec_t* in, vec_t* out );
...
void R_AliasSetupSkin( void )
{
	...
	r_palette = (word*)((byte*)paliashdr + paliashdr->palette);
}
...
void xR_AliasSetupFrame( void )
{
	...
	if (paliashdr->frames[frame].type == ALIAS_SINGLE)
	{
		...
		if (frame != currententity->prevframe)
		{
			currententity->prevanimtime = cl.time;
			currententity->prevframe = frame;
		}

		if (currententity == &cl.viewent || !r_dointerp || (currententity->prevframe >= pmdl->numframes))
		{
			r_apverts = (trivertx_t*)
				((byte*)paliashdr + paliashdr->frames[frame].frame);
			return;
		...
		fullinterval = (cl.time - currententity->prevanimtime) * 10.0;
		...
		prevframe = paliashdr->frames[(int)currententity->prevframe].frame;

		t1 = (trivertx_t*)((byte*)paliashdr + paliashdr->frames[frame].frame);
		t2 = (trivertx_t*)((byte*)t1 + prevframe - paliashdr->frames[frame].frame);

		for (i = 0; i < pmdl->numverts; i++, t1++, t2++)
		{
			s1 = t1->v[0] * fullinterval;
			s2 = t2->v[0] * scale;
			iterpverts[i].v[0] = s1 + s2;
			s1 = t1->v[1] * fullinterval;
			s2 = t2->v[1] * scale;
			iterpverts[i].v[1] = s1 + s2;
			s1 = t1->v[2] * fullinterval;
			s2 = t2->v[2] * scale;
			iterpverts[i].v[2] = s1 + s2;
			iterpverts[i].lightnormalindex = t1->lightnormalindex;
		}
